...
const REGION_GUIDE = {
  "美国":"综合评估（holistic）：成绩、活动主线、文书、推荐信都重要；本科有 ED/EA 早申策略。",
  "英国":"高度学术导向：本科看 A-Level/IB 预估成绩与个人陈述（PS），硕士看本科院校、均分与专业相关性；几乎不看课外活动，没有 ED/EA，不要提到早申/活动连续性。",
  "加拿大":"偏门槛逻辑：GPA 与语言达标是基础，部分项目看研究/相关经历；没有美式早申。",
  "澳洲":"门槛逻辑为主：以 GPA 与语言对照项目分数线，录取相对清晰，活动权重低，没有早申。",
  "新加坡":"竞争激烈、学术为先：GPA、标化与专业匹配是核心，名额少。",
  "中国香港":"看重 GPA 与专业匹配，竞争激烈，多为滚动录取，建议尽早递交；活动权重低于美国。"
};
function regionNote(p){const r=(p&&p.targetRegions&&p.targetRegions[0])||"美国";return `\n本次申请目标地区为「${r}」，请严格按该地区的录取逻辑来判断与建议：${REGION_GUIDE[r]||REGION_GUIDE["美国"]}\n（不要套用其它国家的逻辑，例如非美国地区不要提 ED/EA、不要强调美式课外活动主线。）`;}
...
4. 不推销人工咨询、不做中介导流。
5. 语气专业、直接、克制，使用中文。
6. **品牌口吻**：需要自称时用"录取雷达"，不要自称"AI""人工智能""作为AI""模型"；不要出现"AI 建议/AI 认为"这类说法，直接给判断即可。
...
async function followUp(payload) {
  ...
  const messages = [
    ...
    { role: "user", content:
`请根据用户已填信息，生成 2–3 个最关键的追问问题（${lvl}申请，重点围绕：${focus}）。${regionNote(p)}
...
${J(payload.verdicts || [])}${regionNote(p)}
...
请严格按以下 JSON 输出（dimension 必须用这些精确名称：${JSON.stringify(dims)}；schoolName 必须用这些精确名称：${JSON.stringify(schools)}）。所有内容都要落到这名学生的真实背景与目标专业上，具体、可执行、不重复：
{
 "profileScan":{"highlights":["这名学生最值得一提的加分项1（具体）","加分项2"],"redFlags":["明显的风险点/红旗1（如专业不对口、定量薄弱、缺相关实习、双非等，具体）","红旗2"]},
 ...
 "schoolAnalysis":[{"schoolName":"<精确名称>","whyThisVerdict":"为什么是这个判断（结合该生背景与该校特点）","strengths":["优势1","优势2"],"weaknesses":["短板1","短板2"],"applicationAdvice":"针对此校的具体建议","improvementPath":["想把这所从当前档位提升一档，需要做到的具体动作1","动作2"]}],
 ...
 "backgroundResources":[{"area":"实习/课程/项目/作品/竞赛 之一","action":"具体要补什么、做成什么样","where":"去哪做/投什么岗位/上什么类型课程（给方向，不需真实链接）"}],
 "essayStrategy":{"mainNarrative":"string","whyMajor":"string","whySchool":"string"},
 "essayAngles":[{"title":"文书选题角度标题","hook":"开篇切入点","useExperience":"用你哪段经历来支撑","appealTo":"想让招生官看到你的什么"}],
 "recommendationAdvice":"string",
 "careerOutlook":{"directions":["该专业方向的典型就业出路1","出路2","出路3"],"roi":"用一段话讲这个方向的就业前景与投入产出，面向家长，帮助预算与决策判断"},
 "parentFriendlySummary":{"summary":"string","riskExplanation":"string","budgetAndDecisionAdvice":"string"}
}
essayAngles 给 2–3 个；backgroundResources 给 3–5 条；profileScan 的 highlights 与 redFlags 各 2–4 条。` },
  ];
  let out;
  try {
...
// ---------- 4) 从追问回答中抽取对结构化字段的"更正/补充" ----------
// 让追问真正回馈到评分：例如用户在追问里说"GPA 填错了，其实是 4.2"，这里抽成 {gpaValue:"4.2"}
async function extractCorrections(payload) {
  const p = payload.profile || {};
  const messages = [
    { role: "system", content: RULES },
    { role: "user", content:
`用户先填写了表单，又回答了追问。请对比"原始填写"与"追问回答"，找出追问里**明确更正或新补充**的、会影响申请评估的结构化信息，输出为 JSON。
规则：
1. 只输出用户在追问里**确实提到、且与原填写不同或更准确**的字段；没有变化就不要输出该字段。
2. 不要臆造数据，不确定就不要输出。
3. 数值类（GPA、语言、标化、GRE/GMAT）按用户更正后的值输出字符串。
4. 经历类（实习/项目/活动/科研）如果追问里补充了更具体内容，可输出更完整文本，便于更准确评估。
...
{"corrections":{"gpaValue":"","languageScore":"","standardizedTestScore":"","greGmatScore":"","internships":"","researchExperience":"","projects":"","activities":"","awards":""}}` },
  ];
  const out = await chat(messages, { model: MODEL_FAST });
  if (!out || typeof out !== "object") throw new Error("BAD_SHAPE");
