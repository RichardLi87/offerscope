...
const HOST = (process.env.ZPAY_HOST || "https://z-pay.cn").replace(/\/$/, "");
...
const PRODUCTS = {
  ...
  premium_report:  { name: "OfferScope 完整申请规划报告", price: 49 },
};
...
function createOrder({ assessmentId, productType, channel, email }) {
  ...
  const params = {
    ...
    notify_url: SITE + "/api/pay/notify", return_url: SITE + "/api/pay/return?out_trade_no=" + outTradeNo,
...
function handleNotify(params) {
  ...
  if (String(params.money) !== String(o.money)) return { ok: false, reason: "money_mismatch" };
